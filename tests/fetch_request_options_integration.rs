use mdget::fetch::request::{merge_cookies, parse_cookie, parse_header, validate_bearer};
...
async fn fetch_with_cookies_merged() {
    ...
    let options = FetchOptions {
        request: RequestOptions {
            cookies: vec!["session=abc123_xyz".to_string(), "theme=dark".to_string()],
            ..Default::default()
        },
        ..Default::default()
    };
...
fn test_header_validation_parser() {
    // Valid headers
    assert_eq!(
        parse_header("X-Header: value").unwrap(),
        ("X-Header".to_string(), "value".to_string())
    );
    assert_eq!(
        parse_header("Authorization:  Bearer 123 ").unwrap(),
    ...
    assert_eq!(
        parse_header("Header:Name: value").unwrap(),
        ("Header".to_string(), "Name: value".to_string())
    );
...
fn test_cookie_merging() {
    ...
    assert_eq!(
        merge_cookies(&["a=1".to_string(), "b=2".to_string()]),
        Some("a=1; b=2".to_string())
    );
}

#[test]
fn test_cookie_validation_parser() {
    assert_eq!(parse_cookie("session=abc123").unwrap(), "session=abc123");
    assert_eq!(parse_cookie(" theme=dark ").unwrap(), "theme=dark");

    assert!(parse_cookie("missing-separator").is_err());
    assert!(parse_cookie(" =novalue").is_err());
    assert!(parse_cookie("name=bad\nvalue").is_err());
}

#[test]
fn test_bearer_validation() {
    assert!(validate_bearer("token-123").is_ok());
    assert!(validate_bearer("   ").is_err());
    assert!(validate_bearer("bad\r\ntoken").is_err());
}
